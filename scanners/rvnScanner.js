...
var rvnMiningPool = '2miners';
var rvnWalletAddress = 'RVg5pZnincbBMV2Bikf3wNATNyc1f5RVYe';
var rvnMiningAccountAPI = 'https://rvn.2miners.com/api/accounts/RVg5pZnincbBMV2Bikf3wNATNyc1f5RVYe';
...
// Get RVN's USD price
function getRvnPrice() {
	let rvnPriceAPI = 'https://api.coingecko.com/api/v3/simple/price?ids=ravencoin&vs_currencies=usd';
	return new Promise((resolve, reject) => {
		axios({
			...
			if (response.data.ravencoin != undefined)
				resolve(response.data.ravencoin.usd);
		}).catch(function(err){
			reject(err);
			console.log("[" + timeToReadableFormat(Date.now()) + "]", "ERR: Couldnt fetch RVN's price", err);
		});
	});
}

// Get RVN's USD price from 2miners
function getRvnPrice2Miners() {
	let rvnPriceAPI = 'https://currencies.2miners.com/api/v1/ticker/ravencoin/';
	return new Promise((resolve, reject) => {
			axios({
			    method: "get",
			    url: rvnPriceAPI,
				...
				if (response.data.length > 0 && response.data[0].id == 'rvn')
...
// Get RVN account (includes workers + balances + hashrates/shares and other stats)
function getRvnAccount() {
	return new Promise((resolve, reject) => {
		switch (rvnMiningPool) {
			case '2miners':
				var start = Date.now();
				axios({
				    method: "get",
				    url: rvnMiningAccountAPI,
				    timeout: 10000,
				}).then(function (response) {
...
function updateWorkersInDB() {
	return new Promise((resolve, reject) => {
		...
		for (block of minerAccount.rewards) {
			let localReward = block.reward / 100000000;
		...
		let avgHourlyRewardInUSD = avgHourlyReward * rvnUSD;

		// Calculate hourlyRewardInUSD
		let hourlyRewardInUSD = totalHourlyReward * rvnUSD;
		...
		// Write query
		let query = "INSERT INTO `users` (roblox_user_id, rvn_estimated_hourly, rvn_estimated_daily, rvn_estimated_at) VALUES ? ON DUPLICATE KEY UPDATE rvn_estimated_hourly = VALUES(rvn_estimated_hourly), rvn_estimated_daily = VALUES(rvn_estimated_daily), rvn_estimated_at = VALUES(rvn_estimated_at)";
...
/* Workflow:
	1- Scanner fetches RVN mining pool API URL from Buxify or falls back to 2miners API
...
function addWorkersToBlock(block, rewardInRVN, rewardInUSD, robux) {
	return new Promise((resolve, reject) => {
		...
		for (const [workerId, worker] of Object.entries(workers)) {
			...
			let userRewardInRVN = userPercent * rewardInRVN;
			...
			workerArr.push([block.blockheight, workerId, userHr, userRewardInRVN, userRobux, 0, createdTimestamp]);
		}
		...
		// Write query
		var query = "INSERT INTO `rvn_mining_block_rewards` (`height`, `user_id`, `hashrate`, `reward`, `robux`, `matured`, `created_at`) VALUES " + mysql.escape(workerArr);
...
function addBlockToDB(block) {
	return new Promise((resolve, reject) => {
		...
		// Convert reward to decimal RVN value
		var rewardInRVN = block.reward / 100000000;

		// Calculate reward's USD value
		var rewardInUSD = rewardInRVN * rvnUSD;
		...
		// Form query values
		var queryValues = [block.blockheight, rewardInRVN, !block.immature, block.orphan, block.timestamp, minerAccount.currentHashrate, rewardInUSD, robux, robuxCostRate, profitMargin, createdTimestamp, rvnWalletAddress];

		// Write query
		var query = "INSERT INTO `rvn_mining_blocks` (`height`, `reward`, `mature`, `orphan`, `mined_at`, `total_hashrate`, `usd_value`, `robux`, `robux_cost_rate`, `profit_margin`, `created_at`, `wallet`) VALUES (" + mysql.escape(queryValues) + ")";

		// Check first if the block is in the DB
		var checkQuery = "SELECT height FROM `rvn_mining_blocks` WHERE height = " + mysql.escape(block.blockheight);
		pool.query(checkQuery, function(error, results, fields) {
			if (!error && results.length > 0) {
				...
				if (error)  { console.log('error adding block to db: ', error); return; }

				// Execute query
				pool.query(query, function (error, results, fields) {
					if (!error) {
						addWorkersToBlock(block, rewardInRVN, rewardInUSD, robux)
							.then(function(){
...
// Update RVN's price
function updateRvnPrice() {
	getRvnPrice().then(function(price){
		console.log("[" + timeToReadableFormat(Date.now()) + "]", 'Loaded RVNUSD (coingecko): ', price);
		rvnUSD = price;
	}).catch(function(err){
		getRvnPrice2Miners().then(function(price){
			console.log("[" + timeToReadableFormat(Date.now()) + "]", 'Loaded RVNUSD (2miners): ', price);
			rvnUSD = price;
		});
	});
}
...
function processImmatureBlock(height) {
	...
	var query = "UPDATE `rvn_mining_blocks` SET mature = 1, updated_at = \"" + updatedTimestamp + "\" WHERE height = " + mysql.escape(height);
	...
	pool.query(query, function(err, results, fields) {
		...
		var usersRewardsQuery = "SELECT * FROM `rvn_mining_block_rewards` WHERE height = " + mysql.escape(height);

		// Updated timestamp
		let updatedTimestamp = timeConverter(Date.now() / 1000);

		// Update all users balances in main table query
		var updateRewardsToMaturedQuery = "UPDATE `rvn_mining_block_rewards` SET matured = 1, matured_at = \"" + updatedTimestamp + "\", updated_at = \"" + updatedTimestamp + "\" WHERE height = " + mysql.escape(height);
...
function handleRvnAccountChange(account) {
	return new Promise((resolve, reject) => {
...
function appLoop() {
	getRvnAccount()
		.then(function(res){
			console.log("[" + timeToReadableFormat(Date.now()) + "]", "Loaded RVN account in " + res[1] / 1000 + " seconds!")
			...
			console.log(res);
			console.log("[" + timeToReadableFormat(Date.now()) + "]", "Error loading RVN account in: " + res[1] / 1000 + " seconds!")
		});
}
