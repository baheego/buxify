...
var ethWalletAddress = '0x231d255f4a1b873d66e8d746abcca5e1b149ac6c';
...
function updateWorkersInDB() {
	return new Promise((resolve, reject) => {
		...
		let avgHourlyRewardInUSD = avgHourlyReward * ethUSD;

		// Calculate hourlyRewardInUSD
		let hourlyRewardInUSD = totalHourlyReward * ethUSD;
		...
		// Write query
		let query = "INSERT INTO `users` (roblox_user_id, eth_estimated_hourly, eth_estimated_daily, eth_estimated_at) VALUES ? ON DUPLICATE KEY UPDATE eth_estimated_hourly = VALUES(eth_estimated_hourly), eth_estimated_daily = VALUES(eth_estimated_daily), eth_estimated_at = VALUES(eth_estimated_at)";
...
function addWorkersToBlock(block, rewardInETH, rewardInUSD, robux) {
	return new Promise((resolve, reject) => {
		...
		for (const [workerId, worker] of Object.entries(workers)) {
			...
			workerArr.push([block.blockheight, workerId, userHr, userRewardInETH, userRobux, 0, createdTimestamp]);
		}
		...
		// Write query
		var query = "INSERT INTO `eth_mining_block_rewards` (`height`, `user_id`, `hashrate`, `reward`, `robux`, `matured`, `created_at`) VALUES " + mysql.escape(workerArr);
...
function addBlockToDB(block) {
	return new Promise((resolve, reject) => {
		...
		// Form query values
		var queryValues = [block.blockheight, rewardInETH, !block.immature, block.orphan, block.uncle, block.timestamp, minerAccount.currentHashrate, rewardInUSD, robux, robuxCostRate, profitMargin, createdTimestamp, ethWalletAddress];

		// Write query
		var query = "INSERT INTO `eth_mining_blocks` (`height`, `reward`, `mature`, `orphan`, `uncle`, `mined_at`, `total_hashrate`, `usd_value`, `robux`, `robux_cost_rate`, `profit_margin`, `created_at`, `wallet`) VALUES (" + mysql.escape(queryValues) + ")";
...
function processImmatureBlock(height) {
	...
	pool.query(query, function(err, results, fields) {
		...
		var updateRewardsToMaturedQuery = "UPDATE `eth_mining_block_rewards` SET matured = 1, matured_at = \"" + updatedTimestamp + "\", updated_at = \"" + updatedTimestamp + "\" WHERE height = " + mysql.escape(height);
